...
void LevelManager::LoadLevel(const std::string& filepath, TeamManager* teamManager) {
    ...
    mapGridLayer1.assign(gridRows, std::vector<int>(gridCols, 19)); // 19 is floor
    for (int y = 0; y < gridRows; ++y) {
        for (int x = 0; x < gridCols; ++x) {
            // Create a border wall
            if (x == 0 || x == gridCols - 1 || y == 0 || y == gridRows - 1) {
                mapGridLayer1[y][x] = 0; // 0 is wall
            }
        }
    }
    
    mapGridLayer2.assign(gridRows, std::vector<int>(gridCols, -1));
    mapObjectGrid.assign(gridRows, std::vector<MapObjectId>(gridCols, MapObjectId::Empty));
        ...
        for (int layer = 1; layer <= 2; ++layer) {
            ...
            for (int r = -DRAW_PADDING_TILES; r < gridRows + DRAW_PADDING_TILES; ++r) {
                for (int c = -DRAW_PADDING_TILES; c < gridCols + DRAW_PADDING_TILES; ++c) {
                    bool isWall = false;
                    if (r >= 0 && c >= 0 && r < gridRows && c < currentGrid[r].size()) {
                        if (currentGrid[r][c] == 0) isWall = true; // Wall in legacy CSV
                    }
                    ...
                    if (isWall) {
                        ...
                        if (r + 1 >= 0 && r + 1 < gridRows && c >= 0 && c < currentGrid[r+1].size()) {
                            if (currentGrid[r+1][c] == 0) tileBelowIsFloor = false; // Only hide front if the tile immediately below is another wall
                        }
                        if (tileBelowIsFloor) {
