        ...
        if (isCSV) {
            ...
            for (int c = 0; c < line.length(); ++c) {
                ...
                if (type == 'W') tileID = 5; // Map to Wall Face ID
                    ...
                    Vector2 position = { (float)c * 32.0f, (float)mapGridLayer1.size() * 32.0f };
...
bool LevelManager::IsSolidCollision(Rectangle box) const {
    ...
    int minCol = (int)std::floor(box.x / 32.0f);
    int maxCol = (int)std::floor((box.x + box.width) / 32.0f);
    int minRow = (int)std::floor(box.y / 32.0f);
    int maxRow = (int)std::floor((box.y + box.height) / 32.0f);
        ...
        for (int c = minCol; c <= maxCol; ++c) {
            ...
            for (int tileID : tileIDs) {
                if (tileID == 0) {
                    ...
                    // Right 9px solid
                    Rectangle solidPart = { (float)c * 32.0f + 23.0f, (float)r * 32.0f, 9.0f, 32.0f };
                    if (CheckCollisionRecs(box, solidPart)) {
                        return true;
                    }
                } else if (tileID >= 12 && tileID <= 14) {
                    // Left 9px solid
                    Rectangle solidPart = { (float)c * 32.0f, (float)r * 32.0f, 9.0f, 32.0f };
                    if (CheckCollisionRecs(box, solidPart)) {
                        return true;
                    }
                }
